...
//Get single
router.get('/single/:blog/TitleToGet', function(req, res, next) {
    const blogRequest = sampleBlogs.find((blog) => {
  return req.params.blogTitle === blog.title
 })
 res.json({
  success: true, 
  blog: blogRequest
...
router.delete('/delete/:title', function(req, res, next){
    ...
    const blog = sampleBlogs.findIndex((blog) => {
        return blog.title = deleteTitle
    })

    sampleBlogs.splice(blog, 1)
